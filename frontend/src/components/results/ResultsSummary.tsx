...
function pulseCategoryColor(cat: string) {
  if (cat === 'typical_resting_range' || cat === 'normal') return 'text-emerald-600'
  if (cat === 'elevated_for_resting_checkin' || cat === 'elevated' || cat === 'low' || cat === 'below_typical_resting_range') return 'text-amber-600'
...
function StatCard({
  label, value, unit, sub, color, badge,
}: {
  label: string
  value: string
  unit?: string
  ...
  return (
    <div className="rounded-2xl bg-nn-pale-sky p-4">
      ...
      <p className={`mt-1 text-xl font-bold ${color ?? 'text-nn-navy'}`}>
        {value}
        {unit && <span className="text-sm font-normal text-nn-navy-light ml-1">{unit}</span>}
      </p>
      {sub && <p className="mt-0.5 text-[10px] text-nn-navy-light leading-tight">{sub}</p>}
      {badge && (
...
function FutureMetricRow({ label, explanation }: { label: string; explanation: string }) {
  return (
    <div className="flex items-start gap-3 rounded-xl bg-nn-pale-sky/50 px-4 py-3">
      <div className="mt-0.5 flex-shrink-0 flex h-5 w-5 items-center justify-center rounded-full bg-nn-mist">
        <svg viewBox="0 0 12 12" fill="none" stroke="#7a8fb0" strokeWidth="1.4" className="h-3 w-3">
      ...
      <div className="min-w-0">
        ...
        <p className="text-[10px] text-nn-navy-light leading-snug mt-0.5">{explanation}</p>
      </div>
...
function RealResultsSummary({ r }: { r: CheckupResult }) {
  ...
  const hrBpm = cs?.estimated_pulse_bpm ?? r.rppg_analysis.consensus.estimated_pulse_bpm
  const pulseCategory = cs?.pulse_category ?? r.rppg_analysis.consensus.pulse_category
  ...
  const wellnessScore = mwi?.wellness_score ?? r.rppg_analysis.signal_quality.wellness_score
  const retake = rq?.retake_recommended ?? r.rppg_analysis.consensus.retake_recommended
  ...
  // Per-method HRs
  const posHr = hrs?.heart_rate_by_method?.POS ?? r.rppg_analysis.methods.pos.hr_bpm
  const chromHr = hrs?.heart_rate_by_method?.CHROM ?? r.rppg_analysis.methods.chrom.hr_bpm
  const greenHr = hrs?.heart_rate_by_method?.GREEN ?? r.rppg_analysis.methods.green.hr_bpm

  const posChromDiff = ma?.pos_chrom_difference_bpm
  const outliers = ma?.outlier_methods ?? []

  const duration = rq?.recording_duration_seconds ?? r.recording.duration_seconds
    ...
    <div className="space-y-5">
      ...
      <div className="rounded-3xl bg-white p-6 shadow-sm">
        <div className="mb-5 flex items-center gap-3">
          ...
          <div>
            <h2 className="font-semibold text-nn-navy">Camera-Based Wellness Signal</h2>
            <p className="text-xs text-nn-navy-light">
              Session {r.session_id} · {frameCount} frames · {fps.toFixed(1)} fps
            </p>
          </div>
          {/* Confidence badge */}
          <span className={`ml-auto rounded-full px-3 py-1 text-xs font-semibold capitalize border ${
            confidence === 'good' ? 'bg-emerald-50 text-emerald-700 border-emerald-100' :
        ...
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
          ...
          <StatCard
            ...
            value={overallQuality.charAt(0).toUpperCase() + overallQuality.slice(1)}
          ...
          <div className="rounded-2xl bg-nn-pale-sky p-4">
            ...
            <p className={`mt-1 text-xl font-bold ${trendColor(trend as string)}`}>
              {trendIcon(trend as string)} {(trend as string).charAt(0).toUpperCase() + (trend as string).slice(1)}
            </p>
            {hrs?.window_values_bpm && hrs.window_values_bpm.length > 0 && (
              <p className="mt-0.5 text-[10px] text-nn-navy-light">
                {hrs.min_window_bpm?.toFixed(1)} – {hrs.max_window_bpm?.toFixed(1)} bpm range
              </p>
            )}
            <span className="mt-1.5 inline-block rounded-full bg-white px-2 py-0.5 text-[10px] text-nn-navy-light">
              Check-in trend
          ...
          <div className="rounded-2xl bg-nn-pale-sky p-4">
            ...
            <p className="mt-1 text-xl font-bold text-nn-navy">{duration.toFixed(0)}s</p>
            <p className="mt-0.5 text-[10px] text-nn-navy-light">
              {fps.toFixed(1)} fps · {resolution}
            </p>
            <span className={`mt-1.5 inline-block rounded-full px-2 py-0.5 text-[10px] border ${
              faceDetected ? 'bg-emerald-50 text-emerald-700 border-emerald-100' : 'bg-red-50 text-red-700 border-red-100'
            }`}>
          ...
          {/* 6. Retake */}
          <div className={`rounded-2xl p-4 border ${retake ? 'bg-amber-50 border-amber-200' : 'bg-nn-pale-sky border-transparent'}`}>
            <p className="text-xs font-medium text-nn-navy-light">Retake</p>
            <p className={`mt-1 text-xl font-bold ${retake ? 'text-amber-700' : 'text-emerald-600'}`}>
              {retake ? 'Recommended' : 'Not needed'}
            </p>
            {retakeReasons.length > 0 ? (
              <p className="mt-0.5 text-[10px] text-amber-700 leading-tight">{retakeReasons[0]}</p>
            ) : (
              <p className="mt-0.5 text-[10px] text-nn-navy-light">Signal acceptable</p>
            )}
            <span className="mt-1.5 inline-block rounded-full bg-white px-2 py-0.5 text-[10px] text-nn-navy-light">
              {retakeReasons.length} reason{retakeReasons.length !== 1 ? 's' : ''}
            </span>
          </div>
        </div>
        ...
        {mwi && (
          <div className="mt-4 rounded-2xl bg-nn-pale-sky/60 px-4 py-3 space-y-1.5">
            <p className="text-xs font-semibold text-nn-navy">What this means for you</p>
        ...
        <div className="mt-4 rounded-2xl border border-nn-mist bg-nn-pale-sky/50 px-4 py-3">
          ...
          <div className="grid grid-cols-3 gap-2 text-center mb-3">
            {[
              ...
              return (
                <div key={name} className={`rounded-xl px-2 py-2 ${isOutlier ? 'bg-amber-50 border border-amber-200' : 'bg-white'}`}>
                  <p className="text-[10px] text-nn-navy-light font-medium">{name}</p>
                  <p className="text-sm font-bold text-nn-navy">
                    {bpm != null ? bpm.toFixed(1) : '—'}
                  </p>
                  <p className="text-[9px] text-nn-navy-light">bpm</p>
                  {isOutlier && (
                    <span className="text-[8px] text-amber-700 font-medium">outlier</span>
                  )}
                  {!primary && (
                    <span className="text-[8px] text-nn-navy-light">baseline</span>
                  )}
                </div>
              )
            })}
          ...
          {posChromDiff != null && (
            <div className="flex items-center gap-4 text-xs text-nn-navy-light">
              <span>POS/CHROM diff: <strong className="text-nn-navy">{posChromDiff.toFixed(2)} bpm</strong></span>
              <span>Agreement: <strong className={`capitalize ${qualityColor(agreementQuality)}`}>{agreementQuality}</strong></span>
              {outliers.length > 0 && (
                <span>Outliers: <strong className="text-amber-700">{outliers.join(', ')}</strong></span>
              )}
            </div>
          )}
        </div>
        ...
        {showAdvanced && (
          <div className="mt-3 space-y-3">
              ...
              <div className="rounded-xl bg-nn-pale-sky/50 px-4 py-3">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {hrs.window_values_bpm.map((v, i) => (
                    <span key={i} className="rounded-full bg-white px-2.5 py-1 text-xs font-medium text-nn-navy">
                      {v.toFixed(1)}
                    </span>
                  ))}
                </div>
            ...
            {wf && (
              <div className="rounded-xl bg-nn-pale-sky/50 px-4 py-3">
                <p className="text-xs font-semibold text-nn-navy mb-2">rPPG Waveform Statistics</p>
                <div className="grid grid-cols-2 gap-x-6 gap-y-1 text-[11px]">
                  {[
                    ['Dominant freq', wf.dominant_frequency_hz != null ? `${wf.dominant_frequency_hz.toFixed(3)} Hz` : '—'],
                    ['Dominant BPM', wf.dominant_frequency_bpm != null ? `${wf.dominant_frequency_bpm.toFixed(2)} bpm` : '—'],
                    ['SNR-like score', wf.snr_like_score != null ? wf.snr_like_score.toFixed(4) : '—'],
                    ['Valid windows', wf.valid_window_count.toString()],
                    ['Waveform samples', wf.waveform_sample_count?.toLocaleString() ?? '—'],
                    ['Peak power', wf.peak_power?.toExponential(2) ?? '—'],
                  ].map(([lbl, val]) => (
                    <div key={lbl} className="flex justify-between border-b border-nn-mist/60 py-0.5">
            ...
            {/* Recording details */}
            <div className="rounded-xl bg-nn-pale-sky/50 px-4 py-3">
              <p className="text-xs font-semibold text-nn-navy mb-2">Recording Details</p>
              <div className="grid grid-cols-2 gap-x-6 gap-y-1 text-[11px]">
                {[
                  ...
                  <div key={lbl} className="flex justify-between border-b border-nn-mist/60 py-0.5">
                    ...
                    <span className={`font-medium ${
                      lbl === 'Face detected' && val === 'No' ? 'text-red-600' :
                      lbl === 'Multiple faces' && val.includes('Yes') ? 'text-amber-700' :
                      'text-nn-navy'
                    }`}>{val}</span>
                  </div>
                ))}
              ...
              {multipleFaces && (
                <p className="mt-2 text-[10px] text-amber-700">
                  Multiple faces may reduce signal accuracy. Ensure only one person is in frame.
                </p>
              )}
            </div>
          </div>
      ...
      <div className="rounded-3xl bg-white p-6 shadow-sm">
        <SectionHeader title="Future & Unsupported Metrics">
          <span className="text-[10px] text-nn-navy-light">Not available in this prototype</span>
        </SectionHeader>
        ...
        <p className="mt-3 text-[10px] text-nn-navy-light leading-relaxed" style={{ fontFamily: 'var(--font-body)' }}>
          {r.safety.disclaimer}
        </p>
      </div>
    </div>
...
function MockResultsSummary() {
  ...
  const metrics = [
    { label: 'Estimated Pulse', value: `${r.heartRate} bpm`, badge: 'Camera-based wellness signal', color: 'text-nn-deep-blue' },
    { label: 'Signal Quality', value: r.signalQuality, badge: r.lightingNote, color: 'text-emerald-600' },
    { label: 'Check-in Trend', value: r.trend, badge: 'vs. 7-day baseline', color: 'text-nn-navy' },
    { label: 'Wellness Score', value: '82 / 100', badge: 'Estimated only', color: 'text-emerald-600' },
  ]
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="rounded-3xl bg-white p-6 shadow-sm">
        <p className="text-sm font-bold text-nn-navy mb-3">Future &amp; Unsupported Metrics</p>
        <div className="space-y-2">
          <FutureMetricRow label="Respiratory Rate" explanation="May require a supervised multitask model such as BigSmall or additional validated signal processing." />
          <FutureMetricRow label="Blood Pressure" explanation="Camera-only blood pressure estimation requires validated calibration/modeling or cuff integration." />
          <FutureMetricRow label="SpO2 (Blood Oxygen)" explanation="Requires a validated optical sensor or calibrated model. Not available from webcam alone." />
          <FutureMetricRow label="Pulse Wave Velocity" explanation="Requires timing between multiple pulse sites or additional sensors." />
        </div>
      </div>
    </div>
  )
